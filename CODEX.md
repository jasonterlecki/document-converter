...
- Docx underline runs use explicit underline type to render in Word.
- LaTeX and Docx serializers expand <u> tags found in inline text as a fallback.
...
- Local `codex` session file is ignored in git.
...
- Docx worker also re-parses generated docx during debugging to confirm content.
- Docx worker logs IR block counts during debugging.
